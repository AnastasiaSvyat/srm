...
getUplFileByEmail(email: string): Observable<any> {
  let API_URL = `${this.careService.REST_API}/get-uplFile/?email=${email}`;
  ...
  formData.append('email', employee.email);
    let API_URL = `${this.careService.REST_API}/uplFile`;
  ...
  deleteUplFile(id:any): Observable<any> {
    let API_URL = `${this.careService.REST_API}/delete-cv/${id}`;
