...
export class UploadFileService {
  ...
  uploadFile(name: string, cv: File, idEmployee: string): Observable<UploadFile[]> {
    const formData = new FormData();
    formData.append('cv', cv, name);
    formData.append('name', name);
    formData.append('idEmployee', idEmployee);
    const API_URL = `${this.careService.REST_API}/uplFile?idEmployee=${idEmployee}`;
  ...
  deleteUplFile(id: string): Observable<UploadFile> {
